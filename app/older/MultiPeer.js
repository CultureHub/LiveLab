...
//  this.uuid = this._userData.uuid
  ...
  this.user = {
    uuid: this._userData.uuid,
    nickname: '',
    room: '',
    streams: []
  }

  // utility that allows external libraries to use communication channels and listen for events unrelated to signalling
...
MultiPeer.prototype._connectToPeers = function (_t, peers, servers) {
  ...
  peers.filter((id) => id!== this.user.uuid).forEach(function (id) {
    if(this.peers[id]) {
...
MultiPeer.prototype._attachPeerEvents = function (p, _id) {
  ...
  p.on('data', function (id, data) {
    //  console.log('data', id, JSON.parse(data))
