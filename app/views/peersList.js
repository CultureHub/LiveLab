...
const streamDetails = (info) => {
  ...
  if(settings && settings.video) {
    videoSettings = `${settings.video.width}x${settings.video.height} ${Math.round(settings.video.frameRate)}fps`
    videoMute = html `<i
      class="mh1 fas ${info.isVideoMuted ? 'fa-video-slash dark-pink':'fa-video'}" title="">
    </i>`
  }
  if(settings && settings.audio) {
    ...
    mute =
    html `<i
      class="mh1 fas ${info.isAudioMuted ?'fa-microphone-slash dark-pink':'fa-microphone'}" title="">
    </i>`
  }
...
const peerDetails = ( peer, isLocal = false ) =>  html`<div class="pb2">
  <div class="dib w-100 v-top pt2"> ${peer.nickname} ${isLocal?'(you)':''}</div>
  <div class="dib">
