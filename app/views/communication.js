  ...
  //   let state.layout.settings.stretchToFit = state.layout.menu.state.layout.settings.stretchToFit
     ...
     if(state.layout.settings.showCommunicationInfo === true) {
       ...
       if(stream.settings && stream.settings.video) {
         ...
         videoMute = html `<i
           class="mh1 fas ${stream.isVideoMuted ? 'fa-video-slash dark-pink':'fa-video'} ${stream.isLocal ? "pointer" : ""}" title=${stream.isLocal ? "mute your video" : ""} onclick=${stream.isLocal ? ()=>{
       ...
       if(stream.settings && stream.settings.audio) {
         ...
         mute =
         html `<i
           class="mh1 fas ${stream.isAudioMuted ?'fa-microphone-slash dark-pink':'fa-microphone'} ${stream.isLocal ? "pointer" : ""}" title=${stream.isLocal ? "mute your audio" : ""} onclick=${stream.isLocal ? ()=>{
...
//text-shadow: 2px 2px 3px rgba(213, 0, 143, 1);
    ...
    return html`<div class='w-100 h-100 video-container' style=${state.layout.settings.stretchToFit? '' : 'border:1px solid #555;'}>
